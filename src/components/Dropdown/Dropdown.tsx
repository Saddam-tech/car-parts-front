    ...
    return (
        <ul onMouseEnter={() => setFunc(index)} className={!show ? styles.show : styles.dropdown}>
            {children.map((el: children, i: number) => (
                <Link to={el.path.toUpperCase()} onClick={() => dispatchHandler(el)} className={`${!show ? styles.show : styles.children} ${prevActiveIndex?.child?.name === el.name ? styles.active : ""}`} key={i}>{el.name}</Link>
            ))}
        </ul>
    )
}
